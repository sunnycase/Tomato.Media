...
namespace
{
	...
	struct AudioStreamDetector
	{
		...
		static ComPtr<IMFStreamDescriptor> BuildStreamDescriptor(AVStream* stream)
		{
			...
			auto options = LibAVAudioCodecOptions::CreateFromStream(stream);
			ThrowIfFailed(mediaType->SetUnknown(MF_MT_LIBAV_AUDIO_CODEC_OPTIONS, options.Get()));
...
{
	auto ioContext = std::make_shared<MFAVIOContext>(stream, 8192, false);
