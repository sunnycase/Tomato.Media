...
void vorbis_comment_add(vorbis_comment *vc,const char *comment){
  ...
  size_t comment_buffer_size = vc->comment_lengths[vc->comments] + 1;
  vc->user_comments[vc->comments]=_ogg_malloc(comment_buffer_size);
  strcpy_s(vc->user_comments[vc->comments], comment_buffer_size, comment);
...
void vorbis_comment_add_tag(vorbis_comment *vc, const char *tag, const char *contents){
  size_t comment_buffer_size = strlen(tag) + strlen(contents) + 2;
  char *comment=alloca(comment_buffer_size); /* +2 for = and \0 */
  strcpy_s(comment, comment_buffer_size, tag);
  strcat_s(comment, comment_buffer_size, "=");
  strcat_s(comment, comment_buffer_size, contents);
...
char *vorbis_comment_query(vorbis_comment *vc, const char *tag, int count){
  ...
  size_t fulltag_size = taglen + 1;
  char *fulltag = alloca(fulltag_size);

  strcpy_s(fulltag, fulltag_size, tag);
  strcat_s(fulltag, fulltag_size, "=");
